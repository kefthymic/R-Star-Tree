...
public class IndexFile {
    ...
    /**
     * The block0 contains all the information about all the other blocks. More specific, it has:
     *
     * 1. The blockId, which is 2 bytes
     * 2. The totalBlocks in the indexFile. It contains 4 bytes
     * 3. The whereIsTheRoot in the RTree. It is initialized to -1. It contains 4 bytes
     * 4. The numberOfCoordinates in the indexFile. It contains 2 bytes
     * 5. The sizeOfOneEntry. It contains 4 bytes
     * 6. The maxEntriesInOneNode. It contains 4 bytes
     */
     ...
     *
     * More specific every block--node-- except block0 has the below :
     * 1. The blockId, which is 2 bytes
     ...
     * 4. for the above 2*coordinates*8 bytes will be stored an array of doubles that shows the bounds of the boundingBox
    ...
    public boolean deleteAnEntryAndFillItWithTheLastEntryOfTheNode(short nodeId, int positionOfEntry){
        byte[] block0= getBlock((short) 0);
        byte[] node= getBlock(nodeId);
        byte[] temp;

        int offsetOfRecordToDelete;
        int offsetOfLastRecordInThisNode;

        int howManyEntries= ByteBuffer.wrap(node,6,4).getInt();
        int sizeOfOneEntry= ByteBuffer.wrap(block0, 11, 4).getInt();
        ...
        offsetOfRecordToDelete= sizeOfOneEntry*positionOfEntry +10; //in order to find the position of the deleted record
        offsetOfLastRecordInThisNode= sizeOfOneEntry*(howManyEntries-1) +10; //in order to find the position of the deleted record

        for(int i=0;i<sizeOfOneEntry;i++){
            node[offsetOfRecordToDelete]=node[offsetOfLastRecordInThisNode];
            node[offsetOfLastRecordInThisNode]=0;
            offsetOfRecordToDelete++;
            offsetOfLastRecordInThisNode++;
        }

        temp = ByteBuffer.allocate(4).putInt(howManyEntries-1).array(); //update the slotOfHowManyEntries
        System.arraycopy(temp,0,node,6,4);
    ...
    public void updateTheBoundingBoxOfAnEntryOfANode(short nodeId, int positionOfEntry, BoundingBox newBoundingBox ){
        byte[] block0= getBlock((short) 0);
        byte[] node= getBlock(nodeId);
        byte[] temp;
        ArrayList<Double> arrayListOfBoundingBox= newBoundingBox.getBounds();

        int sizeOfOneEntry= ByteBuffer.wrap(block0, 11, 4).getInt();

        int offsetOfRecord= sizeOfOneEntry*positionOfEntry +10; //in order to find the position of the entry we want to change the boundingBox;

        offsetOfRecord+=6;// In order to go to the slot with the first element from the arrayListOfBoundingBox

        for(int i=0;i<2*this.coordinates;i++){
            offsetOfRecord+=8;
            temp = ByteBuffer.allocate(8).putDouble(arrayListOfBoundingBox.get(i)).array(); //slotsOfArrayListOfBoundingBox
            System.arraycopy(temp,0,node,offsetOfRecord,8);
        }
    ...
    public void changeTheRecordIdOfAnEntryOfLeaf(short nodeId, int positionOfEntry, RecordId recordIdOfTheRecordThatBeenDeleted){
        byte[] block0= getBlock((short) 0);
        byte[] node= getBlock(nodeId);
        byte[] temp;
        ...
        temp = ByteBuffer.allocate(2).putShort(recordIdOfTheRecordThatBeenDeleted.getBlock()).array(); //slotOfBlockFromRecordId
        System.arraycopy(temp,0,node,offsetOfRecord,2);

        offsetOfRecord+=2;
        temp = ByteBuffer.allocate(4).putInt(recordIdOfTheRecordThatBeenDeleted.getSlot()).array(); //slotOfSlotOfRecordId
        System.arraycopy(temp,0,node,offsetOfRecord,4);
